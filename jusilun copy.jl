...
function jusilun(Ωm, ΩΛ, Ωk, h, seed, id; N=64, Lbox=256.0, zi=90.0, headless=true)
    ...
    if headless == true
        ...
        npzwrite("./output_data/initialandfinal128.npz", Dict(
            "rho_f" => reshape(view(u_final, 1, :), size(δ)...),
            "rho_bg_f" => ρ_f,
            "rho_i" => reshape(view(u0, 1, :), size(δ)...),
            "rho_bg_i" => ρ_i
        ))
        return nothing
    else
...
jusilun(0.3, 0.7, 0.0, 0.7, 12345, "test"; N=128, Lbox=256.0, zi=90, headless=true);