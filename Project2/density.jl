...
grid_resolution = 100
# Define grid
xs = range(-45, 45, length=grid_resolution)
ys = range(-45, 45, length=grid_resolution)

# Calculate rho on the grid
# We compute r for each (x,y) and then rho(t, r)
rho_data = [rho(t_0, sqrt(x^2 + y^2)) for y in ys, x in xs]

# Create Heatmap
p = heatmap(
    xs, ys, rho_data,
    c = :magma,           # Colormap
    #title = "Density rho(t=$(t_0), r)",
    #xlabel = "x",
    #ylabel = "y",
    aspect_ratio = :equal,
    size = (550, 500)
...
#plot(r_grid, rho(13, r_grid), xlabel="r", ylabel="rho", title="Density Profile")
gtt() = -c^2
...
for i in 0:60
    ...
    plot!(R .* cos.(phi), R .* sin.(phi), aspect_ratio=:equal, linecolor=:white, linealpha=0.7)
...
plot!(xlabel="x (Mpc)", ylabel="y (Mpc)", xlims=(-45, 45), ylims=(-45, 45), title="Ray Tracer: Parallel Incident Rays", legend=false)
