...
# =====================================================================
# 4. DISCRETE CALLBACK WITH MANUAL ROOTFINDING
# =====================================================================
...
function jusilun(h, Ω_m, Ω_Λ, z_i, δ, run_id)
    ...
    # Solve background
    cb_bg = ContinuousCallback(
        (u, t, integ) -> u[2] - 3*H₀_c,
        nothing, terminate!;
        rootfind=SciMLBase.RightRootFind, save_positions=(false, true))
        
    sol_bg = solve(ODEProblem(bg_ode!, [ρ_bg, Θ_bg], (0.0, 1e4), Λ), Tsit5();
                callback=cb_bg, reltol=1e-12, abstol=1e-14, verbose=false)
    t_end = sol_bg.t[end]
    println("Background reaches H=H₀ at t = $(round(t_end, digits=4)) Gyr")

    # Setup the Cache-Friendly 5 x N Matrix (Including Background)
    δ_cells = vcat(vec(δ), 0.0)  
    ...
    # CRITICAL RAM SAVERS: save_positions=(false, false)
    cb_grid = DiscreteCallback(collapse_condition, collapse_affect!, save_positions=(false, false))
    prob = ODEProblem(grid_ode!, u0, (0.0, t_end), p)

    # dense=false prevents the continuous interpolant from taking up RAM
    sol = solve(prob, Tsit5(); callback=cb_grid, reltol=1e-8, abstol=1e-10, 
                save_everystep=false, dense=false, verbose=false)

    # Transpose the 5xN back to Nx5 to match your existing data format
    ...
    final[:,2] .= max.(final[:,2], 0.0)  

    n_cells_real = N_cells - 1  
    n_collapsed  = count(x -> x == 0.0, final[1:n_cells_real, 2])
    H_avg        = dot(final[:,2], final[:,5]) / sum(final[:,5]) / 3 / H₀_c
    println("H_avg/H₀ = $(round(H_avg, digits=4)) | Collapsed: $n_collapsed / $n_cells_real")
