...
for i in range(50):
    ...
    Omega_m_i, Omega_Q_i, Omega_K_i, Omega_L_i, H_i = get_omegas(initial)
    print(f"Density parameters at H = {H_i * lu / tu * c:.5f} \t z = {initial['z_i']}")
    print(f"Omega_m_i = {Omega_m_i:.5f}")
    print(f"Omega_L_i = {Omega_L_i:.5f}")
    print(f"Omega_Q_i = {Omega_Q_i:.5f}")
    print(f"Omega_K_i = {Omega_K_i:.5f}")
    print(f"Omega_T_i = {Omega_m_i + Omega_L_i + Omega_Q_i + Omega_K_i:.5f}\n")

    final = np.load(f"./data/jusilun_output/final_vals_{str(i).zfill(3)}.npz")
    Omega_m_f, Omega_Q_f, Omega_K_f, Omega_L_f, H_f = get_omegas(final)
    print(f"Density parameters at H = {H_f * lu / tu * c:.5f} \t z = 0")
    print(f"Omega_m_f = {Omega_m_f:.5f}")
    print(f"Omega_L_f = {Omega_L_f:.5f}")
    print(f"Omega_Q_f = {Omega_Q_f:.5f}")
    print(f"Omega_K_f = {Omega_K_f:.5f}")
    print(f"Omega_T_f = {Omega_m_f + Omega_L_f + Omega_Q_f + Omega_K_f:.5f}\n")